...
ipcMain.on('execute-queue-action', (event, { videoId, playNext }) => {
    ...
    const code = `
        (function() {
            ...
            if (!app) {
                console.error('Main process executeJavaScript: ytmusic-app element not found');
                return;
            }
            
            const videoId = "${videoId}";
            const insertPosition = "${insertPosition}";
            const isPlaylist = ${isPlaylist};
            const cleanId = "${cleanId}";
            
            console.log('Main world executing queue command:', videoId, 'isPlaylist:', isPlaylist, 'position:', insertPosition);
